...
## "Capa" fullyconnected, modificacion con sesgo (vector de unos)
##
...
classdef fullyConnectedBiased < handle
  properties
  ...
  methods
    ...
    function s=fullyconnectedBiased()
      s.inputsX=[];
      s.inputsW=[];
      s.outputs=[];
      s.gradientX=[];
    ...
    function y=forward(s,W,X)
      if (isreal(W) && ismatrix(W) && isreal(X) && ismatrix(X))
        s.inputsW=W;
        if (columns(X)==1)
          X=[1;X];  #se agrega el sesgo
          s.inputsX=X; #se guardan inputs con sesgo
          s.outputs = W*X; 
        else
          X=[ones(rows(X),1) X]; #se agrega el sesgo
          s.inputsX=X; #se guardan inputs con sesgo
          s.outputs = X*W';
        endif
        y=s.outputs;
